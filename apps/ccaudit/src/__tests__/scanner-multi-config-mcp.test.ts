...
import { scanMcpServers, compareConfigRef } from '@ccaudit/internal';
  ...
  () => {
    ...
    it('aggregation across many configs is deterministic and dedup-safe', async () => {
      ...
      for (const it of quintetItems) {
        ...
        // configRefs must be in deterministic bucket order (project-local → user → system).
        const sorted = [...it.configRefs!].sort(compareConfigRef);
