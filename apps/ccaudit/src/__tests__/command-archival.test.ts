...
describe.skipIf(process.platform === 'win32')(
  'Phase 3.2 — command archival (SC1, SC3)',
  () => {
    let tmpHome: string;
    ...
    function manifestPathFromEnvelope(stdout: string): string {
      ...
      if (!bust?.manifestPath) {
        throw new Error(
          `bust.manifestPath missing from JSON envelope: ${stdout.slice(0, 500)}`,
        );
      }
    ...
    it('SC1: command selected via CCAUDIT_SELECT_IDS archives end-to-end and restores', async () => {
      ...
      const dry = await runCcauditCli(
        ...
        { env: { CCAUDIT_FORCE_TTY: '0', PATH: `${path.join(tmpHome, 'bin')}:${process.env.PATH ?? ''}` } },
      );
      ...
      expect(existsSync(path.join(tmpHome, '.claude', 'commands', 'sc', 'build.md'))).toBe(
        false,
      );
    ...
    it('SC3a: --dry-run --json envelope counts.commands is populated', async () => {
      ...
      const dryPlan = dryEnvelope.changePlan as
    ...
    it('SC3b: --dry-run text-mode output contains a tight commands row matching the locked regex', async () => {
      ...
      const dryText = await runCcauditCli(
        tmpHome,
        ['ghost', '--dry-run', '--yes-proceed-busting'],
        {
          env: {
            CCAUDIT_FORCE_TTY: '0',
            PATH: `${path.join(tmpHome, 'bin')}:${process.env.PATH ?? ''}`,
