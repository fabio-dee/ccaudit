...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
    ...
    // ─────────────────────────────────────────────────────────────────────
    it('SC5: renderRunningProcessMessage output appears verbatim in BOTH the non-interactive and interactive entry preflight stderr', { timeout: 30_000 }, async () => {
    ...
    // ─────────────────────────────────────────────────────────────────────
    it('SC5b: selectedItems survives across entry-preflight retry AND bust-time runBust retry; [PREFLIGHT_DIRTY] marker confirms runBust actually returned running-process', { timeout: 90_000 }, async () => {
      ...
      const markerCount = (stderrBuf.match(/\[PREFLIGHT_DIRTY\] synthetic dirty/g) ?? [])
        .length;
    ...
    // ─────────────────────────────────────────────────────────────────────
    it('SC6: fixture with hook ghosts produces a tab bar that does NOT include HOOKS', { timeout: 30_000 }, async () => {
      await buildManyGhostsFixture(tmpHome, 1);
    ...
    // ─────────────────────────────────────────────────────────────────────
    it('SC7: "Hook archival deferred" appears in text-mode output; is ABSENT under --json, --csv, --quiet, AND --ci', { timeout: 60_000 }, async () => {
