...
export async function waitForPicker(child: ChildProcess, graceMs = 300): Promise<void> {
  ...
  const earlyExitMs = 1_000;
  const startMs = Date.now();
  let delayMs = 50;
  while (child.exitCode === null && Date.now() - startMs < earlyExitMs) {
    await new Promise((r) => setTimeout(r, delayMs));
    delayMs = Math.min(delayMs * 2, 200);
  }
  // Grace period for the TUI to reach its blocking read.
...
/**
 * Poll until the marker appears in stdout, the child exits, or maxWait elapses.
 * THROWS if the marker never appeared (silent timeouts hide regressions).
 *
 * Accepts a string or RegExp as the marker so callers can assert on
 * dynamic patterns (e.g. `tabbed-picker-force-partial-banner` tests).
 */
...
// ── Phase 8 helpers (restore-interactive fixture) ─────────────────────────

const RESTORE_INTERACTIVE_FIXTURE_DIR = path.join(here, '__fixtures__', 'restore-interactive');

/**
 * Stage the v1.5 restore-interactive fixture into `tmpHome` (Plan 08-06, D8-19).
 *
 * Copies two seed JSONL manifests (a SUBSET bust archiving pencil-dev +
 * pencil-review, then a newer FULL bust archiving code-reviewer + a duplicate
 * pencil-review entry) into `<tmpHome>/.claude/ccaudit/manifests/`, substituting
 * the `{{TMPHOME}}` placeholder with the actual `tmpHome` absolute path so
 * every `source_path` / `archive_path` / `selection_filter.ids` resolves
 * correctly.
 *
 * Also populates `<tmpHome>/.claude/ccaudit/archived/agents/` with the three
 * archive files (pencil-dev.md, pencil-review.md, code-reviewer.md) so
 * `executeRestore` actually has something to rename back to source paths.
 *
 * Sets per-file mtime so `discoverManifests` sorts the full (newer) bust
 * first: that's the invariant dedupManifestOps relies on for newer-wins.
 *
 * Does NOT write `<tmpHome>/.claude.json` or session JSONL — callers
 * requiring the preflight's running-Claude check must invoke `buildFakePs`
 * separately, and `.claude.json` is optional for `restore` (unused unless
 * MCP disable ops appear in the manifest — these fixtures contain only
 * archive ops).
 */
export async function stageRestoreInteractiveFixture(tmpHome: string): Promise<void> {
  ...
  const claudeAgentsDir = path.join(tmpHome, '.claude', 'agents');
  await mkdir(manifestsDir, { recursive: true });
  await mkdir(archivedAgentsDir, { recursive: true });
  await mkdir(claudeAgentsDir, { recursive: true });

  // Substitute {{TMPHOME}} placeholder in each fixture manifest, then write
  // into the staged manifests dir.
  const fixtureFiles = [
    'bust-2026-04-17T10-00-00-000Z-aaaa.jsonl',
    'bust-2026-04-18T10-00-00-000Z-bbbb.jsonl',
  ] as const;
  for (const name of fixtureFiles) {
    const src = path.join(RESTORE_INTERACTIVE_FIXTURE_DIR, name);
    const raw = await readFile(src, 'utf8');
    const body = raw.split('{{TMPHOME}}').join(tmpHome);
    await writeFile(path.join(manifestsDir, name), body, 'utf8');
  }

  // Force mtime ordering so `discoverManifests` sorts newer-first:
  // the 2026-04-18 (full) bust must appear before the 2026-04-17 (subset).
  const olderMtime = new Date('2026-04-17T10:00:00.000Z');
  const newerMtime = new Date('2026-04-18T10:00:00.000Z');
  await utimes(
    path.join(manifestsDir, 'bust-2026-04-17T10-00-00-000Z-aaaa.jsonl'),
    olderMtime,
    olderMtime,
  );
  await utimes(
    path.join(manifestsDir, 'bust-2026-04-18T10-00-00-000Z-bbbb.jsonl'),
    newerMtime,
    newerMtime,
  );

  // Seed archive files that the manifests reference. Content is arbitrary
  // (executeRestore moves by rename; sha256 is not re-verified on restore).
  await writeFile(
    path.join(archivedAgentsDir, 'pencil-dev.md'),
    '# pencil-dev (archived)\n',
    'utf8',
  );
  await writeFile(
    path.join(archivedAgentsDir, 'pencil-review.md'),
    '# pencil-review (archived)\n',
    'utf8',
  );
  await writeFile(
    path.join(archivedAgentsDir, 'code-reviewer.md'),
    '# code-reviewer (archived)\n',
    'utf8',
  );

  // Seed a memory file referenced by a FlagOp in the full-bust manifest so
  // the restore picker can surface and restore a MEMORY row. Some tests only
  // need the item visible; the dedicated memory round-trip test also selects
  // it, so the stub frontmatter includes the two ccaudit keys that restore
...
/** Absolute path to the restore-interactive fixture source directory. */
export function restoreInteractiveFixtureDir(): string {
  return RESTORE_INTERACTIVE_FIXTURE_DIR;
}
