...
function parsePickerTokensK(frame: string): number | null {
  // k-form (ASCII or Unicode).
  const kMatch = frame.match(/(?:~|≈)\s+(\d+)k\s+tokens saved/);
  if (kMatch !== null) return parseInt(kMatch[1]!, 10);
  // Raw form: `N tokens saved` where N is a bare integer.
  const rawMatch = frame.match(/\b(\d+)\s+tokens saved/);
