...
export const restoreCommand = define({
  name: 'restore',
  description:
    'Restore items archived by a previous --dangerously-bust-ghosts run',
  ...
  async run(ctx) {
    ...
    try {
      ...
      if (outMode.json) {
        process.stdout.write(
          JSON.stringify(
            buildJsonEnvelope('restore', 'n/a', 2, { error: message }),
          ) + '\n',
        );
      } else {
    ...
    if (outMode.json) {
      process.stdout.write(
        JSON.stringify(
          buildJsonEnvelope(
            ...
            'n/a',
            exitCode,
            restoreResultToJson(result, warnings),
          ),
        ) + '\n',
      );
    } else if (outMode.csv) {
      ...
      process.stdout.write(
        renderRestoreRendered(result, warnings, outMode.verbose ?? false),
      );
    }
  ...
  switch (result.status) {
    ...
    case 'no-manifests':
      return {
        ...
        status: 'no-manifests',
        message:
          'No bust history found. Run ccaudit --dangerously-bust-ghosts first.',
      };
    case 'name-not-found':
      return {
        ...base,
...
function summarizeListEntry(entry: ManifestListEntry): Record<string, unknown> {
  return {
    ...
    items: entry.ops
      .filter(
        (o): o is ArchiveOp | DisableOp =>
          o.op_type === 'archive' || o.op_type === 'disable',
      )
      .map(summarizeOp),
  };
}
...
function renderRestoreRendered(
  ...
  lines.push(
    `${c.unarchived.completed} agent(s)/skill(s) unarchived${c.unarchived.failed > 0 ? ` (${c.unarchived.failed} failed)` : ''}`,
  );
  lines.push(
    `${c.reenabled.completed} MCP server(s) re-enabled${c.reenabled.failed > 0 ? ` (${c.reenabled.failed} failed)` : ''}`,
  );
  lines.push(
    `${c.stripped.completed} memory file(s) stripped${c.stripped.failed > 0 ? ` (${c.stripped.failed} failed)` : ''}`,
  );
  ...
  if (result.status === 'success') {
    lines.push(colorize.green('\u2713 All operations completed.'));
  } else {
    lines.push(
      colorize.yellow(
        `\u26A0 Partial success: ${result.failed} operation(s) failed.`,
      ),
    );
  }
...
function renderListOutput(entries: ManifestListEntry[]): string {
  ...
  const lines: string[] = [];
  lines.push(
    `Archived items \u2014 ${entries.length} bust${entries.length === 1 ? '' : 's'}`,
  );
  lines.push('');

  for (const entry of entries) {
    const isoName = path
      .basename(entry.path, '.jsonl')
      .replace(/^bust-/, '');
...
function renderRestoreCsv(result: RestoreResult): string {
  ...
  if (result.status !== 'success' && result.status !== 'partial-success') {
    ...
    return (
      header +
      `restore,meta,${result.status},,,,${result.status},${error}\n`
    );
  }
...
if (import.meta.vitest) {
  ...
  describe('restoreResultToExitCode', () => {
    const cases: Array<[RestoreResult, number]> = [
      [{ status: 'success', counts: { unarchived: { completed: 1, failed: 0 }, reenabled: { completed: 0, failed: 0 }, stripped: { completed: 0, failed: 0 } }, manifestPath: '/p', duration_ms: 10 }, 0],
      [{ status: 'no-manifests' }, 0],
      [{ status: 'name-not-found', name: 'x' }, 0],
      [{ status: 'list', entries: [] }, 0],
      [{ status: 'partial-success', counts: { unarchived: { completed: 0, failed: 1 }, reenabled: { completed: 0, failed: 0 }, stripped: { completed: 0, failed: 0 } }, failed: 1, manifestPath: '/p', duration_ms: 5 }, 1],
  ...
  describe('renderRestoreQuiet', () => {
    it('success: emits tab-separated status + counts', () => {
      const result: RestoreResult = {
        status: 'success',
        counts: { unarchived: { completed: 2, failed: 0 }, reenabled: { completed: 1, failed: 0 }, stripped: { completed: 3, failed: 0 } },
        manifestPath: '/m',
        duration_ms: 50,
      };
    ...
    it('no-manifests: emits status only', () => {
      expect(renderRestoreQuiet({ status: 'no-manifests' })).toBe(
        'restore\tno-manifests\n',
      );
    });
  });
  ...
  describe('renderRestoreCsv', () => {
    ...
    it('success emits 3 summary rows', () => {
      const result: RestoreResult = {
        status: 'success',
        counts: { unarchived: { completed: 1, failed: 0 }, reenabled: { completed: 1, failed: 0 }, stripped: { completed: 1, failed: 0 } },
        manifestPath: '/m',
        duration_ms: 10,
      };
  ...
  describe('extractServerName (re-export check)', () => {
    ...
    it('handles nested projects path key', () => {
      expect(
        extractServerName('projects./foo.mcpServers.my.server'),
      ).toBe('my.server');
    });
  });
}
