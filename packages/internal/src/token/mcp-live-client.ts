...
// Injected at bundle time by tsdown's define option — never a runtime require.
declare const __CCAUDIT_VERSION__: string;
const CLIENT_VERSION: string = __CCAUDIT_VERSION__;
...
export async function listMcpTools(
  ...
  if (config.type && config.type !== 'stdio') {
    throw new Error(`Live measurement not available for ${config.type} transport. Use estimate.`);
  }
  ...
  return new Promise<McpToolDefinition[]>((resolve, reject) => {
    ...
    // Spawn the MCP server child process with a minimal environment.
    // Passing the full process.env would leak secrets (API keys, tokens, etc.)
    // to third-party MCP server processes. Only forward what's needed for
    // process resolution, plus any vars the server explicitly configured.
    ...
    child.on('exit', (code) => {
      if (!settled && code !== null && code !== 0) {
        settle(() => reject(new Error(`MCP server exited with code ${code}`)));
      }
    });
    ...
    const initRequest =
      JSON.stringify({
        ...
        params: {
          ...
          capabilities: {},
          clientInfo: { name: 'ccaudit', version: CLIENT_VERSION },
        },
      }) + '\n';
...
if (import.meta.vitest) {
  ...
  describe('listMcpTools', () => {
    it('should return parsed tool definitions from a mock MCP server', async () => {
      const tools = await listMcpTools({ command: 'node', args: ['-e', mockMcpServer] }, 10_000);
    ...
    it('should reject for non-stdio transport with clear message', async () => {
      await expect(listMcpTools({ command: 'node', type: 'http' })).rejects.toThrow(
        /http.*transport/i,
      );
    });
