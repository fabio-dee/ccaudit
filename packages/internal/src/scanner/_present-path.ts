...
export function presentPath(absPath: string, homeDir: string, projectRoot?: string): string {
  ...
  if (projectRoot && projectRoot.length > 0 && absPath.startsWith(projectRoot + '/')) {
    return absPath.slice(projectRoot.length + 1);
  }
  // Step 2 — home compression.
  if (homeDir && homeDir.length > 0 && absPath.startsWith(homeDir + '/')) {
    return '~/' + absPath.slice(homeDir.length + 1);
  }
