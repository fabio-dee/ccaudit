 ...
 * when `helpOpen === true`.
 *
 * Content is grouped into functional sections (D5-14):
 *   Navigation / Selection / View / Glyphs / Exit
 *
 ...
 *
 * Phase 9 E4: the column-1 glyph legend is inlined so users can decode
 * colorblind-friendly row state markers without leaving the picker.
 */
...
function buildGroups(useAscii: boolean): readonly Group[] {
  ...
  return [
    ...
    {
      heading: 'Glyphs',
      bindings: buildGlyphLegend(useAscii),
    },
    {
      heading: 'Exit',
      bindings: [
...
function buildGlyphLegend(useAscii: boolean): readonly Binding[] {
  const [selected, unselected, locked, multi, stale] = useAscii
    ? ['[x]', '[ ]', '#', '!', '~']
    : ['◉', '◯', '🔒', '⚠', '⌛'];
  return [
    { keys: selected, desc: 'Selected' },
    { keys: unselected, desc: 'Unselected' },
    { keys: locked, desc: 'Protected / framework-locked' },
    { keys: multi, desc: 'Multi-config MCP server' },
    { keys: stale, desc: 'Stale memory file' },
  ];
}
...
if (import.meta.vitest) {
  ...
  describe('renderHelpOverlay', () => {
    it('contains all group headings and at least one binding each (D5-14)', () => {
    ...
    it('includes Unicode glyph legend matching rendered row glyphs (Phase 9 E4)', () => {
      const out = renderHelpOverlay({ useAscii: false, rows: 30, cols: 100 });
      expect(out).toContain('── Glyphs ──');
      expect(out).toContain('◉');
      expect(out).toContain('Selected');
      expect(out).toContain('◯');
      expect(out).toContain('Unselected');
      expect(out).toContain('🔒');
      expect(out).toContain('Protected / framework-locked');
      expect(out).toContain('⚠');
      expect(out).toContain('Multi-config MCP server');
      expect(out).toContain('⌛');
      expect(out).toContain('Stale memory file');
    });

    it('includes ASCII glyph legend when ASCII mode is active', () => {
      const out = renderHelpOverlay({ useAscii: true, rows: 30, cols: 100 });
      expect(out).toContain('# Glyphs');
