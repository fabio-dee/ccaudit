 ...
 *
 * For stale memory items: glyph is ⌛ (or ~ under ASCII fallback).
...
export function formatRowLabel(item: TokenCostResult, useAscii: boolean, now: number): string {
  ...
  // Memory staleness glyph (D-14, D-15). Recent memory gets no extra marker;
  // stale memory uses the same glyph advertised in the help overlay legend.
  let glyph = '';
  if (category === 'memory' && item.item.mtimeMs !== undefined) {
    const ageDays = (now - item.item.mtimeMs) / 86_400_000;
    const isStale = ageDays > 60;
    if (isStale) {
      glyph = `${useAscii ? GLYPHS_ASCII.staleMemory : GLYPHS_UNICODE.staleMemory} `;
    }
  }
...
if (import.meta.vitest) {
  ...
  describe('selectGhosts (thin adapter)', () => {
    ...
    it('formats stale memory with hourglass by default and tilde in ASCII fallback', () => {
      const now = Date.parse('2026-04-26T00:00:00.000Z');
      const stale = makeGhost({ name: 'CLAUDE.md', category: 'memory', tokens: 4 });
      stale.item.mtimeMs = now - 91 * 86_400_000;

      expect(formatRowLabel(stale, false, now)).toMatch(/^⌛ CLAUDE\.md/);
      expect(formatRowLabel(stale, true, now)).toMatch(/^~ CLAUDE\.md/);
    });
  });
}
