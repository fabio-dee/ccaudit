...
import { GLYPHS_ASCII, GLYPHS_UNICODE } from './_glyphs.ts';
...
export interface TabbedPickerInput {
  ...
  /**
   * Phase 8.1 D81-02 (C1b): optional custom footer renderer. When provided and
   * the picker is NOT in filter mode, substitutes the hard-coded ghost-side
   * `"${selected} of ${total} selected across all tabs"` global-line template.
   * Restore adapter passes `(n, m) => \`${n} selected · ${m} archived\``; ghost
   * side passes nothing → current behavior preserved byte-for-byte.
   */
  renderFooter?: (selected: number, total: number) => string;
  /**
...
interface TabState {
  ...
  cursor: number;
  /**
   * Phase 9 Plan 02 (D3 / SC3): cursor saved on filter-entry and restored
   * on filter-Esc so scroll position survives filter on/off cycles. null
   * when no filter session is active. Mirrors applyScroll's reducer shape
   * in `_viewport.ts` but lives inline on the per-tab struct so the key
   * handler paths can mutate it directly (no Redux-style dispatch here).
   */
...
export class TabbedGhostPicker extends MultiSelectPrompt<FlatOption> {
  ...
  /** D81-02: optional footer override; when set and not in filter mode, replaces global-line. */
  public renderFooter?: (selected: number, total: number) => string;
  ...
  constructor(input: TabbedPickerInput) {
    ...
    this.renderFooter = input.renderFooter;
    ...
    this.on('key', (char, info) => {
      ...
      if (this.filterMode) {
        ...
        if (info?.name === 'escape') {
          ...
          // Phase 9 Plan 02 (D3 / SC3): restore pre-filter cursor so
          // scroll position survives filter on/off toggles.
          const tab = this.tabs[this.activeTabIndex];
          if (tab && tab.savedCursorPreFilter !== null) {
            tab.cursor = tab.savedCursorPreFilter;
        ...
        if (info?.name === 'return') {
          ...
          // Keep savedCursorPreFilter while the submitted filter remains
          // active. A later Esc in normal picker mode clears the filter first
          // and restores/clamps this cursor instead of canceling the picker.
          this.state = 'active';
          return;
        }
      ...
      // Phase 9 D2: in normal picker mode, Esc clears an already-submitted
      // active filter before it can cancel the picker. A second Esc (with no
      // active filter) still reaches the base cancel path.
      if (info?.name === 'escape') {
        const st = this.filterSortByTab[this.activeTabIndex];
        if (st?.active === true && st.query !== '') {
          st.query = '';
          st.active = false;
          const tab = this.tabs[this.activeTabIndex];
          if (tab && tab.savedCursorPreFilter !== null) {
      ...
      if (char === '/') {
        ...
        if (st) {
          ...
          st.active = true;
          // Phase 9 Plan 02 (D3 / SC3): save pre-filter cursor for
          // Esc-restore. Idempotent — don't overwrite outer save.
          const tab = this.tabs[this.activeTabIndex];
          if (tab && tab.savedCursorPreFilter === null) tab.savedCursorPreFilter = tab.cursor;
        }
        this.state = 'active';
        return;
      }
    ...
    (
      ...
      const keyInfo = info as
        ...
        | undefined;
      const activeFilter = this.filterSortByTab[this.activeTabIndex];
      const activeSubmittedFilter =
        activeFilter?.active === true && activeFilter.query !== '' && !this.filterMode;
      const suppressing = this.filterMode || this.helpOpen || activeSubmittedFilter;
      const isEscape = keyInfo?.name === 'escape';
      const isReturn = keyInfo?.name === 'return';
      if (suppressing && (isEscape || (isReturn && (this.filterMode || this.helpOpen)))) {
  ...
  public cursorPageUp(): void {
    ...
    const vh = computeViewportHeight({
      ...
      bannerRows: bannerHeight({
        active: this.forcePartial,
        protectedCount: this._countProtected(),
        ascii: this.useAscii,
   ...
   * Under in-source vitest, fails loud to surface regressions; in
  ...
  /**
   * Phase 6 Plan 03 (NEW-M3): count items flagged as framework-protected
   * across all tabs. Extracted from the inline loop in `_renderFrame` so
   * the same figure can be passed to `bannerHeight` without an O(items)
   * scan on every PageUp/PageDown keystroke.
   */
  private _countProtected(): number {
    let count = 0;
    for (const item of this.itemsById.values()) {
      if (isProtected(item.item)) count++;
    }
    return count;
  }
  ...
  public _registerResize(): void {
    ...
    // Phase 9 Plan 02 (D5 / SC5): trailing-edge 50ms throttle coalesces
    // SIGWINCH bursts on macOS Terminal.app drag-resize. Bind on both
    // stdout 'resize' and process 'SIGWINCH' so piped stdio still ticks.
    const handler = (): void => {
      if (this._resizeThrottleTimer !== null) clearTimeout(this._resizeThrottleTimer);
  ...
  public _renderFrame(): string {
    ...
    if (visible.length === 0 && activeState?.active === true && activeState.query !== '') {
      ...
      // Phase 9 Plan 02 (D4 / SC4): resolve glyphs once per frame.
      const glyphs = this.useAscii ? GLYPHS_ASCII : GLYPHS_UNICODE;
      for (let i = 0; i < win.slice.length; i++) {
        ...
        if (row.kind === 'sub-header') {
          ...
          const marker = isSelected ? glyphs.selected : glyphs.unselected;
    ...
    if (this.filterMode) {
      ...
      lines.push(
        pc.dim(
          `Tab ${leftArrow} ${rightArrow} tabs ${dot} ${upArrow}${downArrow} nav ${dot} / search ${dot} ? help ${dot} Space toggle ${dot} a tab-all ${dot} Enter ${rightArrow} ${dot} q cancel`,
        ),
      );
    }
    ...
    if (filterActiveHere) {
      ...
      // D81-02 (C1b): when a custom renderFooter is provided (restore path),
      // substitute its output for the hard-coded ghost-side template. Ghost
      // side leaves renderFooter undefined → byte-identical behavior.
      const globalLine =
        this.renderFooter !== undefined
          ? this.renderFooter(this.selectedIds.size, totalItems)
          : counterDisplay === ''
...
if (import.meta.vitest) {
  ...
  // Phase 9 Plan 02 (D5 / SC5) — SIGWINCH listener lifecycle tests.
  describe('SIGWINCH listener lifecycle (T-09-05 mitigation)', () => {
    it('register + unregister is balanced: process SIGWINCH listener count returns to baseline', () => {
      const before = process.listenerCount('SIGWINCH');
      const picker = makePicker([makeGhost({ name: 'a1', category: 'agent', tokens: 1 })]);
    ...
    it('double-register is idempotent (second call is a no-op)', () => {
      const before = process.listenerCount('SIGWINCH');
      const picker = makePicker([makeGhost({ name: 'a1', category: 'agent', tokens: 1 })]);
      picker._registerResize();
      picker._registerResize();
    ...
    it('double-unregister is safe (second call is a no-op)', () => {
      const before = process.listenerCount('SIGWINCH');
      const picker = makePicker([makeGhost({ name: 'a1', category: 'agent', tokens: 1 })]);
      picker._registerResize();
      picker._unregisterResize();
  ...
  describe('TabbedGhostPicker', () => {
    ...
    describe('D81-02 renderFooter prop', () => {
      it('ghost path (renderFooter undefined): global-line byte-identical to existing template', () => {
        const ghosts = [
          makeGhost({ name: 'a1', category: 'agent', tokens: 100 }),
          makeGhost({ name: 's1', category: 'skill', tokens: 200 }),
        ];
        ...
        const frame = picker._renderFrame();
        expect(frame).toContain('0 of 2 selected across all tabs');
        // Canonical restore template must NOT appear on ghost path.
        expect(frame).not.toContain('archived');
      });

      it('restore path (renderFooter defined): substitutes injected template, ghost template absent', () => {
        const ghosts = [
          makeGhost({ name: 'a1', category: 'agent', tokens: 100 }),
          makeGhost({ name: 's1', category: 'skill', tokens: 200 }),
        ];
        ...
        const frame = picker._renderFrame();
        expect(frame).toContain('0 selected \u00B7 2 archived');
        // Ghost template must be fully replaced — not appended.
        expect(frame).not.toContain('of 2 selected across all tabs');
      });
    });
    ...
    describe('Phase 5 filter + sort — Task 2 key bindings + footer', () => {
      ...
      it('Esc after submitted filter clears filter without canceling and restores saved cursor', () => {
        const ghosts = [
          makeGhost({ name: 'alpha', category: 'agent' }),
          makeGhost({ name: 'beta', category: 'agent' }),
          makeGhost({ name: 'alpine', category: 'agent' }),
        ];
        const picker = makePicker(ghosts);
        picker.tabs[0]!.cursor = 2;
        fireKey(picker, '/');
        fireKey(picker, 'a');
        fireKey(picker, 'l');
        fireKey(picker, undefined, { name: 'return' });
        expect(picker.filterMode).toBe(false);
        expect(picker.filterSortByTab[0]!.active).toBe(true);
        expect(picker.tabs[0]!.savedCursorPreFilter).toBe(2);

        fireKey(picker, undefined, { name: 'escape' });
        expect(picker.filterSortByTab[0]!.query).toBe('');
        expect(picker.filterSortByTab[0]!.active).toBe(false);
        expect(picker.tabs[0]!.cursor).toBe(2);
        expect(picker.state).toBe('active');
      });
